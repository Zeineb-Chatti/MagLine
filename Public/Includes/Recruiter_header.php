<?php
// Update path to match candidate style
$headerProfilePicture = isset($_SESSION['manager_photo']) 
    ? '/Public/Uploads/Manager_Photos/' . $_SESSION['manager_photo']
    : '/Public/Assets/default-user.png'; // Keep absolute path for consistency

$headerRecruiterName = ($_SESSION['user_name'] ?? 'Recruiter') . ' ' . ($_SESSION['user_lastname'] ?? '');
?>
    ...
    <div class="header-actions">
        ...
        <div class="profile-dropdown">
            <div class="profile-dropdown-toggle">
                <img src="<?= htmlspecialchars($headerProfilePicture) ?>" alt="Profile" class="profile-avatar" 
                     onerror="this.onerror=null;this.src='/Public/Assets/default-user.png'">
                <span class="profile-name"><?= htmlspecialchars($headerRecruiterName) ?></span>
                <i class="bi bi-chevron-down"></i>
            </div>
            <div class="profile-dropdown-menu">
                <a href="settings.php"><i class="bi bi-gear me-2"></i> Settings</a>
                <a href="company_profile.php"><i class="bi bi-building me-2"></i> Company Profile</a>
                <a href="Logout.php?role=recruiter"><i class="bi bi-box-arrow-right me-2"></i> Logout</a>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getNotificationUrl(notif) {
        ...
        console.log('Notification type received:', notif.type);
        switch (notif.type) {
            case 'application': // Handle database type for new applications
                return `Applications.php?id=${notif.related_id}`;
            case 'status_change': // Handle database type for status changes
                return `application-detail.php`;
            case 'job_update': // Handle job updates
                return `Recruiter_job_details.php?id=${notif.related_id}`;
            case 'system': // Handle system notifications (e.g., new job posted)
                return `Recruiter_job_details.php?id=${notif.related_id}`;
            case 'interview_scheduled': // Keep for future use if added to database
                return `Recruiter_interview_details.php?id=${notif.related_id}`;
            case 'job_post_approved': // Keep for future use
            case 'job_post_rejected':
                return `Recruiter_job_details.php?id=${notif.related_id}`;
            case 'new_message': // Keep for future use
                return `Recruiter_messages.php`;
            case 'system_alert': // Keep for future use
                return `Recruiter_dashboard.php`;
            default:
                console.warn('No URL mapping for notification type:', notif.type);
                return null;
        }
    }
    ...
    async function loadUnreadMessagesCount() {
        try {
            ...
            if (data.success) {
                ...
                updateMessagesBadge(1); // Test value
            }
        } catch (error) {
